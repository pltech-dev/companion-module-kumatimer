export function setupActions(sendCommand) {
    return {
        start: {
            name: 'Start',
            options: [],
        ...
        load_time: {
            ...
            callback: async (action) => sendCommand('load_time', { seconds: Number(action.options['seconds']) }),
        },
        load_time_mmss: {
        ...
        preset: {
            ...
            callback: async (action) => sendCommand('preset', { index: Number(action.options['index']) }),
        },
        load_cue: {
            name: 'Load Cue',
            options: [{ type: 'number', id: 'index', label: 'Cue index (0 = first)', default: 0, min: 0, max: 999 }],
            callback: async (action) => sendCommand('load_cue', { index: Number(action.options['index']) }),
        },
        next_cue: {
            name: 'Next Cue',
            options: [],
        ...
        set_mode: {
            ...
            callback: async (action) => sendCommand('set_mode', { mode: action.options['mode'] }),
        },
        send_sms: {
